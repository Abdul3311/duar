...
let handler = async (m, { conn }) => {
  ...
  const cap = `🧑🏻‍🏫  ɴᴀᴍᴇ: ${conn.getName(m.sender)}
...
`.trim()
let tum = await (await fetch(pp)).buffer()
let tumbb = await conn.resize(tum, 300, 300)
conn.sendMessage(m.chat, { image: tum, 
caption: cap, 
footer: author,
mentions: await conn.parseMention(m.sender),
templateButtons: [  
{ urlButton: { displayText: 'Website', url: 'https://lynk.id/ZansLord' }},
{ quickReplyButton: { displayText: 'Dompet', id: `.dp` }},
{ quickReplyButton: { displayText: 'Bank', id: `.bank` }}
]
}, { quoted: m }) 
}
