...
let handler = async (m, { conn, isOwner, usedPrefix: prefix, command, args }) => {
if (!args[0]) return m.reply(`Silahkan masukkan nomor!\nContoh: ${prefix}${command} +${m.sender.split`@`[0]}`)
if (args[0].startsWith('0')) return m.reply(`Awali nomor dengan +62!\nContoh: ${prefix}${command} +${m.sender.split`@`[0]}`)
if (args[0].startsWith('8')) return m.reply(`Awali nomor dengan +62!\nContoh: ${prefix}${command} +${m.sender.split`@`[0]}`)
