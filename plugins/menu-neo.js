...
switch (template) {
            ...
            case 'animeget':
        let ab = await fetch(`https://api.neoxr.my.id/api/anime-get?url=${one}&apikey=vremiumz`)
...
${ac.stream[3].url.url}`)
            break
            case 'anime':
        let bb = await fetch(`https://api.neoxr.my.id/api/anime?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, bf, be, m)
            break
            case 'artinama':
        let cb = await fetch(`https://api.neoxr.my.id/api/artinama?nama=${one}&apikey=vremiumz`)
...
case 'papercut':
case 'slice':
        let nsf = `https://api.neoxr.my.id/api/${args[0]}?text=${one}&apikey=vremiumz`
        conn.sendButtonImg(m.chat, nsf, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'gores':
        let db = await fetch(`https://api.neoxr.my.id/api/gore-search?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, df, de, m)
            break
            case 'getgore':
        let eb = await fetch(`https://api.neoxr.my.id/api/gore-get?url=${one}&apikey=vremiumz`)
	...
	conn.sendButtonVid(m.chat, ec.data.video, ee, 'Nih.mp4', 'To Sticker', '.s', fakes, adReply)
            break
            case 'gore':
        let fb = await fetch(`https://api.neoxr.my.id/api/gore?q=${one}&apikey=vremiumz`)
	...
	conn.sendButtonVid(m.chat, fc.data.video, fe, 'Nih.mp4', 'To Sticker', '.s', fakes, adReply)
        ...
        let gb = await fetch(`https://api.neoxr.my.id/api/ig?url=${one}&apikey=vremiumz`)
        let gc = await gb.json()
            conn.sendButtonImg(m.chat, gc.data[0].url, gc.data[0].type, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'igstalk':
        let hb = await fetch(`https://api.neoxr.my.id/api/igstalk?username=${one}&apikey=vremiumz`)
...
${hc.data.username}
	...
	conn.sendButtonVid(m.chat, giflogo, he, 'Nih.mp4', 'To Sticker', '.s', fakes, adReply)
            ...
            case 'igstory':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let ib = await fetch(`https://api.neoxr.my.id/api/igstory?username=${one}&apikey=vremiumz`)
        let ic = await ib.json()
            conn.sendButtonImg(m.chat, ic.data[0].url, ic.data[0].type, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'line':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let jb = await fetch(`https://api.neoxr.my.id/api/line?url=${one}&apikey=vremiumz`)
        let jc = await jb.json()
            conn.sendButtonImg(m.chat, jc.data.url, author, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'mediafire':
        let kb = await fetch(`https://api.neoxr.my.id/api/mediafire?url=${one}&apikey=vremiumz`)
...
${kc.data.uploaded}
	...
	conn.sendButtonVid(m.chat, giflogo, ke, 'Nih', 'Get', '.get ' + kc.data.link, fakes, adReply)
            ...
            case 'pin':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let lb = await fetch(`https://api.neoxr.my.id/api/pin?url=${one}&apikey=vremiumz`)
        let lc = await lb.json()
            conn.sendButtonVid(m.chat, lc.data.url, lc.data.type, 'Nih', 'Get', '.get ' + lc.data.link, fakes, adReply)
            break
            case 'pinterest':
        let mb = await fetch(`https://api.neoxr.my.id/api/pinterest?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, mf, me, m)
            break
            case 'podcast':
        let nb = await fetch(`https://api.neoxr.my.id/api/podcast?url=${one}&apikey=vremiumz`)
...
${nc.data.title}
	...
	conn.sendButtonVid(m.chat, giflogo, ne, 'Nih', 'Get', '.get ' + nc.data.audio, fakes, adReply)
            break
            case 'rexdlget':
        let ob = await fetch(`https://api.neoxr.my.id/api/rexdl-get?url=${one}&apikey=vremiumz`)
...
${oc.data.url}
	...
	conn.sendButtonVid(m.chat, giflogo, oe, 'Nih', 'Get', '.get ' + oc.data.url, fakes, adReply)
            ...
            case 'rexdl':
        let pb = await fetch(`https://api.neoxr.my.id/api/rexdl?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, pf, pe, m)
            ...
            case 'shorten':
        let qb = await fetch(`https://api.neoxr.my.id/api/shorten?url=${one}&apikey=vremiumz`)
        let qc = await qb.json()
        m.reply('Result: ' + qc.data.url)
                break
                case 'soundclouds':
        let rb = await fetch(`https://api.neoxr.my.id/api/soundcloud-search?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, rf, re, m)
            ...
            case 'soundcloud':
        let sb = await fetch(`https://api.neoxr.my.id/api/soundcloud?url=${one}&apikey=vremiumz`)
...
${sc.data.author.verified}`
	conn.sendButtonVid(m.chat, sc.data.imageURL, se, 'Nih', 'Get', '.get ' + sc.data.url, fakes, adReply)
            ...
            case 'ss':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let tb = `https://api.neoxr.my.id/api/ss?url=${one}&apikey=vremiumz`
            conn.sendButtonImg(m.chat, tb, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'sshp':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let ub = `https://api.neoxr.my.id/api/sshp?url=${one}&apikey=vremiumz`
            conn.sendButtonImg(m.chat, ub, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            
            case 'sticker':
        let vb = await fetch(`https://api.neoxr.my.id/api/sticker?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, vf, ve, m)
            ...
            case 'stickerget':
        let wb = await fetch(`https://api.neoxr.my.id/api/sticker-get?url=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, wf, we, m)
            ...
            case 'swiki':
        let xb = await fetch(`https://api.neoxr.my.id/api/swiki?q=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, xf, xe, m)
            break
            case 'swikiget':
        let yb = await fetch(`https://api.neoxr.my.id/api/swiki-get?url=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, yf, ye, m)
            ...
            case 'telesticker':
        let zb = await fetch(`https://api.neoxr.my.id/api/telesticker?url=${one}&apikey=vremiumz`)
	...
	return conn.sendListM(m.chat, zf, ze, m)
            ...
            case 'tiktok':
        let Ab = await fetch(`https://api.neoxr.my.id/api/tiktok?url=${one}&apikey=vremiumz`)
        let Ac = await Ab.json()
...
${Ac.data.caption}`
	conn.send2ButtonVid(m.chat, Ac.data.videoWM, Ae, 'Nih', 'No WM', '.get ' + Ac.data.video, 'MP3', '.get ' + Ac.data.audio, fakes, adReply)
            ...
            case 'tiktokfull':
        let Bb = await fetch(`https://api.neoxr.my.id/api/tiktokfull?url=${one}&apikey=vremiumz`)
        let Bc = await Bb.json()
...
${Bc.downloads[2].url}`
	conn.send2ButtonVid(m.chat, Bc.downloads.url, Be, 'Nih', 'No WM', '.get ' + Bc.downloads[1].url, 'MP3', '.get ' + Bc.downloads[2].url, fakes, adReply)
            break
            case 'twitter':
        let Cb = await fetch(`https://api.neoxr.my.id/api/twitter?url=${one}&apikey=vremiumz`)
        let Cc = await Cb.json()
    let Ce = `${Cb.author}
${Cb.like}
${Cb.retweet}
...
${Cb.data[0].url}`
	conn.sendButtonVid(m.chat, Cc.data[0].url, Ce, 'Nih', 'Get', '.get ' + Cb.data[0].url, fakes, adReply)
            ...
            case 'urban':
        let Db = await fetch(`https://api.neoxr.my.id/api/urban?q=${one}&apikey=vremiumz`)
        let Dc = await Db.json()
        m.reply('Result:\n' + Dc.data.content + '\n' + Dc.data.author)
                break
                
                case 'wallpaper':
        let Eb = await fetch(`https://api.neoxr.my.id/api/wallpaper?q=${one}&apikey=vremiumz`)
        let Ec = await Eb.json()
    let Ed = Ec.data
	...
	return conn.sendListM(m.chat, Ef, Ee, m)
            break
            case 'wallpaper2':
        let Fb = await fetch(`https://api.neoxr.my.id/api/wallpaper2?q=${one}&apikey=vremiumz`)
        let Fc = await Fb.json()
    let Fd = Fc.data
  ...
  if (/webp/g.test(b_)) d_ = await webp2png(c_)
        ...
        let wnt = `https://api.neoxr.my.id/api/whatanime?url=${d_}&apikey=vremiumz`
